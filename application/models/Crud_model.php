...
class Crud_model extends CI_Model {
  ...
  function get_listing_wise_review($listing_id = "") {
    return $this->db->get_where('review', array('listing_id' => $listing_id))->row_array();
  }
  ...
  function get_rating_preshsure_rating($listing_id = "") {
      $this->db->select_avg('preshsure_rating');
      $rating = $this->db->get_where('review', array('listing_id' => $listing_id))->row()->preshsure_rating;
      return number_format((float)$rating, 0, '.', '');
  }
  function get_rating_privacy_rating($listing_id = "") {
      $this->db->select_avg('privacy_rating');
      $rating = $this->db->get_where('review', array('listing_id' => $listing_id))->row()->privacy_rating;
      return number_format((float)$rating, 0, '.', '');
  }
  ...
  function post_review() {
      ...
      $this->db->where('id', $id);
      $this->db->update('review', $updater);
  }
    function post_review_update($listing_id) {
   
      $data['review_rating'] = sanitizer($this->input->post('cleanliness'));
      $data['preshsure_rating'] = sanitizer($this->input->post('preshsure'));
...
function add_listing() {
  ...
  $data['name'] = sanitizer($this->input->post('location'));
  $data['description'] = sanitizer($this->input->post('description'));

  $data['first_name'] = sanitizer($this->input->post('f_name'));
  $data['last_name'] = sanitizer($this->input->post('l_name'));
  $data['is_spray'] = sanitizer($this->input->post('is_spray'));
  $data['handicap'] = sanitizer($this->input->post('handicap'));
  ...
  $data['comments'] = sanitizer($this->input->post('comments'));
  ...
  foreach ($_FILES['listing_images']['tmp_name'] as $listing_image) {
    if ($listing_image != "") {
      ...
      $this->createThumbnail($random_identifier);
...
// - makes the function easier to read
 ...
 public function createThumbnail($filename)
   {
      $source_path = FCPATH . 'uploads/listing_images/' . $filename;
      $target_path = FCPATH . 'uploads/listing_images/thumb-' . $filename;
      // echo FCPATH . 'uploads/listing_images/thumb-' . $filename;
      // exit();
  ...
  foreach ($_FILES['listing_images']['tmp_name'] as $key => $listing_image) {
    ...
    if (in_array($new_listing_images[$key], $old_listing_images)) {
      ...
      if ($listing_image != "") {
        ...
        $this->createThumbnail($random_identifier);
      // echo FCPATH.'uploads/listing_images/'.$random_identifier;
      // exit();
...
$this->post_review_update($listing_id);
...
public function get_listing_by_name($name) {
